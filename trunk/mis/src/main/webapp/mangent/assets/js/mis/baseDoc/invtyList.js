...
function search2() {
	...
	$.ajax({
		...
		contentType: 'application/json; charset=utf-8',
		success: function(data) {
//			if(data.reqHead.isSuccess==false){
//				alert(data.reqHead.message)
//			}
			var arr = eval(data); //数组
			var list = arr.respBody.list;
			var arr1 = [];
...
$(document).on('click', '.true', function() {
	...
	if(ids.length == 0) {
		...
		var encds = encd.join(',');
		var nms = nm.join(',');
		window.parent.opener.document.getElementById("invtyEncd").value = encds;
		window.parent.opener.document.getElementById("invtyNm").value = nms;
		window.close()
	}
})
$(document).on('click', '.false', function() {
	window.parent.opener.document.getElementById("invtyEncd").value = "";
	window.parent.opener.document.getElementById("invtyNm").value = "";
	window.close()
})