	...
	$(".re_true").click(function() {
		...
		if(encd.length == 0) {
			...
			for(var i = 0; i < encd.length; i++) {
				encd[i].isNtRtnGoods = 0;
				$("#jqgrids").setRowData(i + 1, {
					sellOrdrIds: encd[i].delvSnglId,
...
function searchDelvSnglId() {
	...
	$("input[name='sellOrdrId']").val(rowDatas.delvSnglId); //销售订单
...
$(function() {
	...
	$("#jqgrids").jqGrid({
		url: '../../assets/js/json/order.json',
		datatype: "json",
		colNames: ['来源单据号', '序号', '仓库编码', '仓库名称', '存货编码', '存货名称', '规格型号', '主计量单位', '主计量单位编码', '数量', '含税单价', '价税合计',
			'无税单价', '无税金额', '税率', '税额', '箱规', '箱数', '对应条形码', '批次', '生产日期', '失效日期', '保质期', '国际批次', '表体备注', '是否退货'
		], //jqGrid的列显示名字
		colModel: [{
				name: 'sellOrdrIds',
				editable: true,
...
function SaveModifyData() {
	...
	stlSnglId = $("input[name='stlSnglId']").val();
	var stlSnglDt = $("input[name='stlSnglDt']").val(); //销售日期
	var custId = $("#custId").val();
	...
	if(IsCheckValue(custId, bizTypId, accNum, listData) == true) {
		...
		$.ajax({
			url: url + '/mis/purc/EntrsAgnStl/editEntrsAgnStl',
			type: 'post',
			data: saveData,
			dataType: 'json',
			async: true,
...
function SaveNewData() {
	...
	if(IsCheckValue(custId, bizTypId, accNum, listData) == true) {
		...
		var saveData = JSON.stringify(savedata);
		$.ajax({
			url: url + '/mis/purc/EntrsAgnStl/addEntrsAgnStl',
			type: 'post',
			data: saveData,
			dataType: 'json',
			async: true,
			...
			success: function(data) {
				alert(data.respHead.message)
				stlSnglId = data.respBody.stlSnglId;
				$("input[name='stlSnglId']").val(data.respBody.stlSnglId); //订单编码
				if(data.respHead.isSuccess == true) {
					$(".addOrder").removeClass("gray");
					$(".saveOrder").addClass("gray");
					$(".editOrder").removeClass("gray");
...
function chaxun() {
	...
	stlSnglId = localStorage.stlSnglId;
	var savedata = {
		"reqHead": reqhead,
		"reqBody": {
			"stlSnglId": stlSnglId,
	...
	$.ajax({
		type: "post",
		contentType: 'application/json; charset=utf-8',
		url: url + '/mis/purc/EntrsAgnStl/queryEntrsAgnStl',
		async: true,
		data: saveData,
		dataType: 'json',
		success: function(data) {
			...
			$("input[name='stlSnglDt']").val(list.stlSnglDt);
			$("input[name='stlSnglId']").val(list.stlSnglId);
			...
			$("#sellOrdrId").val(list.sellOrdrId)
			$("input[name='memo']").val(list.memo);
			$(".custOrdrNum").val(list.custOrdrNum)

			var arr = eval(data); //数组
			var list1 = arr.respBody.entrsAgnStlSub;
			$("#jqgrids").jqGrid('clearGridData');
			$("#jqgrids").jqGrid('setGridParam', {
				datatype: 'local',
