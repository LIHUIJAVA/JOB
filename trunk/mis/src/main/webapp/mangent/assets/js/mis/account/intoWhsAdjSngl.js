$(function() {
	//点击表格图标显示收发类别
	$(document).on('click', '.recv_biaoge', function() {
	...
	$(document).on('click', '.provr_biaoge', function() {
...
$(function() {
	$('.editOrder').click(function() {
		...
		if(b == 1) {
			...
			getPursOrdrId(formNum);
		}
	});
})

//得到入订单号之后的操作
function getPursOrdrId(formNum) {
	var savedata = {
		"reqHead": reqhead,
		"reqBody": {
			"formNum": formNum,
	...
	$.ajax({
		type: "post",
		contentType: 'application/json; charset=utf-8',
		url: url + '/mis/purc/PursOrdr/queryPursOrdrList',
		async: true,
		data: saveData,
		dataType: 'json',
		success: function(data) {
			var list = data.respBody.list[0].formNum; //有错
			for(var i = 0; i < list.length; i++) {
...
function IsCheckValue(provrId,recvSendCateId, listData) {
...
function SaveModifyData() {
	...
	if(IsCheckValue(provrId,recvSendCateId, listData) == true) {
