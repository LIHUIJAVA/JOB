...
$(".ui-pg-selbox.ui-widget-content.ui-corner-all").click(function(){
	...
	var postD3 = JSON.stringify(data3);
		jQuery("#jqgrids").jqGrid({			
			url : url3+"/mis/ec/orderStatus/queryList",//组件创建完成之后请求数据的url
			mtype:"post",
			...
			colNames : [ '状态编码','状态名称','备注'],//jqGrid的列显示名字
				colModel : [ //jqGrid每一列的配置信息。包括名字，索引，宽度,对齐方式.....
				   {name : 'orderStatusId',align:"center",index : 'invdate',editable:true,},
				   {name : 'orderStatusName',align:"center",index : 'id',editable:true,},				             
			...
			sortname : 'orderStatusId',//初始化的时候排序的字段
...
//条件查询
$('#find').click(function(){
		var orderStatusId =$('input[name="orderStatusId1"]').val();
	...
	var postD2 = JSON.stringify(data2);
		$('#jqgrids').jqGrid('setGridParam',{
			url:url3+"/mis/ec/orderStatus/queryList",
			mtype:"post",
			datatype : "json",//请求数据返回的类型。可选json,xml,txt
			postData:postD2,
			rowNum:rowNum,
...
jQuery("#jqgrids").jqGrid({	
			url : url3+"/ec/orderStatus/queryList",//组件创建完成之后请求数据的url
			mtype:"post",
			datatype : "json",//请求数据返回的类型。可选json,xml,txt
			postData:postD3,
			ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
			colNames : [ '状态编码','状态名称','备注'],//jqGrid的列显示名字
			colModel : [ //jqGrid每一列的配置信息。包括名字，索引，宽度,对齐方式.....
			 	{name : 'orderStatusId',index : 'invdate',editable:false,},
				{name : 'orderStatusName',index : 'id',},				             
...
$(function(){
	$(".addOrder").click(function(){
			...
			$("#jqgrids").setColProp('orderStatusId',{editable:true});//设置editable属性由true改为false
	...
	$(".saveOrder").click(function(){
    	var orderStatusId = $("input[name='orderStatusId']").val();
    	var orderStatusName = $("input[name='orderStatusName']").val();
		...
		$.ajax({
			type:"post",
			url:url3+"/mis/ec/orderStatus/add",
			async:true,
			data:saveJson,
			dataType:'json',
...
$(function(){
    $(".delOrder").click(function(){
		...
		var deleteAjax = {			
			reqHead,
			"reqBody":{	
				"orderStatusId":rowDatas.orderStatusId,
				"orderStatusName":rowDatas.orderStatusName,
				"memo":rowDatas.memo,
			}			
		};	
		...
		if(gr == null ){
			...
			$.ajax({
				type:"post",
				url:url3+"/mis/ec/orderStatus/delete",
				async:true,
				data:deleteData,
				dataType:'json',
				contentType: 'application/json',
...
//初始化表格
	jQuery("#jqgrids").jqGrid({
				...
				colNames : [ '状态编码','状态名称','备注'],//jqGrid的列显示名字
				colModel : [ //jqGrid每一列的配置信息。包括名字，索引，宽度,对齐方式.....
				   {name : 'orderStatusId',align:"center",index : 'invdate',editable:false,},
				   {name : 'orderStatusName',align:"center",index : 'id',editable:true,},				             
				   {name : 'memo',align:"center",index : 'invdate',editable:true,},
		...
		ondblClickRow: function(){
			...
			$(".update").click(function(){
				var orderStatusId=rowDatas.orderStatusId;
				var orderStatusName = $("input[name='orderStatusName']").val();
				var memo = $("input[name='memo']").val();
				   ...
				   var edit = {
				    	...
				    	$.ajax({
				    		type:"post",
							url:url3+"/mis/ec/orderStatus/edit",
							async:true,
							data:editJson,
							dataType:'json',
							contentType: 'application/json',
