...
public class Sign {
	...
	public static String signTopRequestKaoLa(Map<String, String> params, String secret, String signMethod)
			throws IOException, NoSuchAlgorithmException {
		// 第一步：检查参数是否已经排序
		String[] keys = params.keySet().toArray(new String[0]);
		Arrays.sort(keys);
		...
		//query.append("clientSecret");//收尾用secret包住
		// 第三步：使用MD5/HMAC加密
		byte[] bytes;
		if ("hmac".equals(signMethod)) {
			bytes = encryptHMAC(query.toString(), secret);
		...
		// 第四步：把二进制转化为大写的十六进制(正确签名应该为32大写字符串，此方法需要时使用)
		System.err.println("SIGN:"+byte2hex(bytes));
		return byte2hex(bytes);
	}
