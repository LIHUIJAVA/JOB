...
$(function() {
	jQuery("#insert_jqgrids").jqGrid({
		...
		pager: '#insert_jqGridPager', //表格页脚的占位符(一般是div)的id
		...
		onPaging: function(pageBtn) { //翻页实现 
			var records = $("#insert_jqgrids").getGridParam('records'); //获取返回的记录数
			page = $("#insert_jqgrids").getGridParam('page'); //获取返回的当前页
			var rowNum1 = $("#insert_jqgrids").getGridParam('rowNum'); //获取显示配置记录数量
...
function search1() {
	...
	$.ajax({
		...
		success: function(data) {
			...
			$("#insert_jqgrids").jqGrid("clearGridData");
			$("#insert_jqgrids").jqGrid("setGridParam", {
				data: mydata.rows,
				jsonReader: {
...
function search2() {
	...
	var invtyEncd1 = $("input[name='invtyEncd1']").val()
	var myData = {};
	var Data = {
		"reqHead": reqhead,
		"reqBody": {
...
$(document).on('click', '.true', function() {
	//获得行号
	var gr = $("#insert_jqgrids").jqGrid('getGridParam', 'selrow');
	//获得行数据
	var rowDatas = $("#insert_jqgrids").jqGrid('getRowData', gr);
	window.parent.opener.document.getElementById("momKitNm").value = rowDatas.invtyNm;

	localStorage.setItem("invtyEncd", rowDatas.invtyEncd);
	window.close();
})
