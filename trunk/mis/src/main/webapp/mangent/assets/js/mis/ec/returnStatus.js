...
$(".ui-pg-selbox.ui-widget-content.ui-corner-all").click(function(){
	...
	var postD3 = JSON.stringify(data3);
		jQuery("#jqgrids").jqGrid({			
			url : url3+"/mis/ec/returnStatus/queryList",//组件创建完成之后请求数据的url
			mtype:"post",
			...
			colNames : [ '状态编码','状态名称','备注'],//jqGrid的列显示名字
				colModel : [ //jqGrid每一列的配置信息。包括名字，索引，宽度,对齐方式.....
				   {name : 'returnStatusId',align:"center",index : 'invdate',editable:true,},
				   {name : 'returnStatusName',align:"center",index : 'id',editable:true,},				             
			...
			sortname : 'returnStatusId',//初始化的时候排序的字段
...
//条件查询
$('#find').click(function(){
		var returnStatusId =$('input[name="returnStatusId1"]').val();
	...
	var postD2 = JSON.stringify(data2);
		$('#jqgrids').jqGrid('setGridParam',{
			url:url3+"/mis/ec/returnStatus/queryList",
			mtype:"post",
			datatype : "json",//请求数据返回的类型。可选json,xml,txt
			postData:postD2,
			rowNum:rowNum,
...
jQuery("#jqgrids").jqGrid({	
			url : url3+"/ec/returnStatus/queryList",//组件创建完成之后请求数据的url
			mtype:"post",
			datatype : "json",//请求数据返回的类型。可选json,xml,txt
			postData:postD3,
			ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
			colNames : [ '状态编码','状态名称','备注'],//jqGrid的列显示名字
			colModel : [ //jqGrid每一列的配置信息。包括名字，索引，宽度,对齐方式.....
			 	{name : 'returnStatusId',index : 'invdate',editable:false,},
				{name : 'returnStatusName',index : 'id',},				             
...
$(function(){
	$(".addOrder").click(function(){
			...
			$("#jqgrids").setColProp('returnStatusId',{editable:true});//设置editable属性由true改为false
	...
	$(".saveOrder").click(function(){
    	var returnStatusId = $("input[name='returnStatusId']").val();
    	var returnStatusName = $("input[name='returnStatusName']").val();
		...
		$.ajax({
			type:"post",
			url:url3+"/mis/ec/returnStatus/add",
			async:true,
			data:saveJson,
			dataType:'json',
...
$(function(){
    $(".delOrder").click(function(){
		...
		var deleteAjax = {			
			reqHead,
			"reqBody":{	
				"returnStatusId":rowDatas.returnStatusId,
				"returnStatusName":rowDatas.returnStatusName,
				"memo":rowDatas.memo,
			}			
		};	
		...
		if(gr == null ){
			...
			$.ajax({
				type:"post",
				url:url3+"/mis/ec/returnStatus/delete",
				async:true,
				data:deleteData,
				dataType:'json',
				contentType: 'application/json',
...
//初始化表格
	jQuery("#jqgrids").jqGrid({
				...
				colNames : [ '状态编码','状态名称','备注'],//jqGrid的列显示名字
				colModel : [ //jqGrid每一列的配置信息。包括名字，索引，宽度,对齐方式.....
				   {name : 'returnStatusId',align:"center",index : 'invdate',editable:false,},
				   {name : 'returnStatusName',align:"center",index : 'id',editable:true,},				             
				   {name : 'memo',align:"center",index : 'invdate',editable:true,},
		...
		ondblClickRow: function(){
			...
			$(".update").click(function(){
				var returnStatusId= rowDatas.returnStatusId;
				var returnStatusName = $("input[name='returnStatusName']").val();
				var memo = $("input[name='memo']").val();
				   ...
				   var edit = {
				    	...
				    	$.ajax({
				    		type:"post",
							url:url3+"/mis/ec/returnStatus/edit",
							async:true,
							data:editJson,
							dataType:'json',
							contentType: 'application/json',
