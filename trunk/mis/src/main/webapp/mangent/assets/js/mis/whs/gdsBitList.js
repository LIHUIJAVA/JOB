...
function search7() {
	...
	if(localStorage.getItem("whsEncd") != null) {
		var whsEncd = localStorage.getItem("whsEncd")
	}
	var Data = {
		'reqHead': reqhead,
		"reqBody": {
	...
	$.ajax({
		type: "post",
		url: url + "/mis/whs/gds_bit_distion/selectWhsgds",
		async: true,
		data: changeData,
		dataType: 'json',
...
$(function() {
	...
	jQuery("#jqGrids").jqGrid({
		...
		shrinkToFit: false,
		colNames: ['仓库编码', '仓库名称','区域编码', '区域名称','货位编码', '货位名称'
		],
		colModel: [{
				name: 'whsEncd',
				editable: false,
...
$(function(){
	$(".true").click(function() {
		...
		if(rowDatas.gdsBitEncd==undefined){
			alert("请选择单据")
		}else{
