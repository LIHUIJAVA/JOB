    ...
    return {
        ...
        alert: function (options) {
            ...
            options = $.extend(true, {
                ...
                ico: "" // put icon before the message
            }, options);

            var id = App.getUniqueID("App_alert");

            var html = '<div id="' + id + '" class="custom-alerts alert alert-' + options.type + ' fade in">' + (options.close ? '<button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button>' : '') + (options.ico !== "" ? '<i class="fa-lg fa fa-' + options.ico + '"></i>  ' : '') + options.message + '</div>';
...
//          console.log(basePath);
        ...
        setGlobalImgPath: function (path) {
            globalImgPath = path;
//          console.log(globalImgPath); //dist/img
        },
            ...
            if (typeof data[i].divider !== 'undefined') {
                ...
                if (typeof data[i].subMenu !== 'undefined') {
                    ...
                    if (typeof data[i].icon !== 'undefined')
                        element += '<span class="glyphicon ' + data[i].ico + '"></span> ';
            ...
            $.each(data, function (i, item) {
                ...
                if (item.children && item.children.length > 0) {
                    ...
                    {   
                        ...
                        var href = 'addTabs({id:\'' + item.id + '\',title: \'' + item.name + '\',close: true,url: \'' + item.url + '\',urlType: \'' + item.urlType + '\'});';
                        $a.attr('onclick', href);
                    }
...
$(function() {
	...
	$.ajax({
		...
		success: function(data) {
			var myname = data.respBody.userName;
			var myDept = data.respBody.depName;
			localStorage.setItem("loginUserName",myname)
			var nameDept = myname +"-"+myDept;
			
			$(".user-menu .dropdown-toggle .hidden-xs").text(myname);
			$(".navbar-nav>.user-menu>.dropdown-menu>li.user-header>p").text(nameDept);
			$(".navbar-nav>.user-menu>.dropdown-menu>li.user-header>p").append("<small>"+loginTime+"</small>");
		},
		error:function(){
			alert("登陆时间失效")
		}
