...
//	allHeight();
	$("#jqgrids").jqGrid({
		datatype: "local",
		colNames: ['序号', '供应商分类编码', '供应商分类名称', '采购科目编码', '采购科目名称', '采购税金编码', '采购税金科目'], //jqGrid的列显示名字
		colModel: [{
				name: 'autoId',
				editable: true,
				hidden: false,
...
function loadLocalData() {
	...
	$.ajax({
		...
		url: url + '/mis/account/taxSubj/selectTaxSubj',
		async: true,
		data: saveData,
		dataType: 'json',
...
function grr(rowid) {
	...
	$("#" + rowid + "_pursSubjNm").attr("readonly", "readonly");
	$("#" + rowid + "_pursTaxNm").attr("readonly", "readonly");
	$("#" + rowid + "_provrClsEncd").attr("readonly", "readonly");
	$("#" + rowid + "_pursSubjEncd").attr("readonly", "readonly");
	$("#" + rowid + "_pursTaxEncd").attr("readonly", "readonly");
	/*打开弹框*/
	...
	$("#" + rowid + "_pursSubjEncd").bind("dblclick", function() {
		add = 2;
		$(".account_List").css("opacity", 1);
		$(".account_List").show();
		$("#purchaseOrder").hide();
	});
	$("#" + rowid + "_pursTaxEncd").bind("dblclick", function() {
		add = 3;
		$(".account_List").css("opacity", 1);
		$(".account_List").show();
		$("#purchaseOrder").hide();
...
$(function() {
	$(".sure").click(function() {
		...
		if(add == 1) {
			...
			$("#" + gr + "_pursSubjEncd").val(addId);
			$("#" + gr + "_pursSubjNm").val(addName);
		} else if(add == 3) {
			var idd = $("#jqGrids").jqGrid('getGridParam', 'selrow');
			var rowData = $("#jqGrids").jqGrid('getRowData', idd);
			addId = rowData.subjId;
			addName = rowData.subjNm;
			$("#" + gr + "_pursTaxEncd").val(addId);
			$("#" + gr + "_pursTaxNm").val(addName);
		}
	})
...
function SaveNewData() {
	...
	var pursSubjEncd = $("#" + newId + '_pursSubjEncd').val();
	var pursTaxEncd = $("#" + newId + '_pursTaxEncd').val();
	var provrClsNm = $("#" + newId + '_provrClsNm').val();
	var pursSubjNm = $("#" + newId + '_pursSubjNm').val();
	var pursTaxNm = $("#" + newId + '_pursTaxNm').val();
	...
	var savedata = {
		...
		"reqBody": {
			...
			'pursSubjEncd': pursSubjEncd,
			'pursTaxEncd': pursTaxEncd,
			'provrClsNm': provrClsNm,
			'pursSubjNm': pursSubjNm,
			'pursTaxNm': pursTaxNm,
		}
	...
	var saveData = JSON.stringify(savedata);
	$.ajax({
		url: url + '/mis/account/taxSubj/insertTaxSubj',
		type: 'post',
		data: saveData,
		dataType: 'json',
		async: true,
...
function SaveModifyData() {
	...
	for(var i = 0; i < gr.length; i++) {
		var obj = {};
		obj.autoId = $("#" + gr[i] + '_autoId').val()
		obj.provrClsEncd = $("#" + gr[i] + '_provrClsEncd').val()
		obj.pursSubjEncd = $("#" + gr[i] + '_pursSubjEncd').val()
		obj.pursTaxEncd = $("#" + gr[i] + '_pursTaxEncd').val();
		obj.provrClsNm = $("#" + gr[i] + '_provrClsNm').val();
		obj.pursSubjNm = $("#" + gr[i] + '_pursSubjNm').val();
		obj.pursTaxNm = $("#" + gr[i] + '_pursTaxNm').val();
		rowDatas[i] = obj;
	}
	var savedata = {
		"reqHead": reqhead,
	...
	if(gr.length == 0) {
		...
		$.ajax({
			url: url + '/mis/account/taxSubj/updateTaxSubj',
			type: 'post',
			data: saveData,
			dataType: 'json',
			async: true,
...
$(function() {
	$(".delOrder_a").click(function() {
		...
		for(i = 0; i < gr.length; i++) {
			//选中行的数据
			var jstime = $("#jqgrids").getCell(gr[i], "autoId");
			rowDatas[i] = jstime
		}
		...
		if(rowData.length == 0) {
			...
			$.ajax({
				type: "post",
				url: url + "/mis/account/taxSubj/deleteTaxSubj",
				async: true,
				data: delData,
				dataType: 'json',
...
$(function() {
	$(".importExcel").click(function() {
		...
		if(files != "") {
			$.ajax({
				type: 'post',
				url: url + "/mis/account/taxSubj/uploadFileAddDb",
				data: data,
				dataType: "json",
...
$(document).on('click', '.exportExcel', function() {
	...
	$.ajax({
		url: url + '/mis/account/taxSubj/selectTaxSubjPrint',
		type: 'post',
		data: Data,
		dataType: 'json',
		async: true,
