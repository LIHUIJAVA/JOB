...
function ondbClick() {
	...
	var data = {
		"reqHead": reqhead,
		"reqBody": {
			'realWhs': rowDatas.realWhs,
		}
	};
	var postData = JSON.stringify(data);
	$.ajax({
		url: url + '/mis/whs/whs_doc/selectRealWhs',
		type: 'post',
		data: postData,
		dataType: 'json',
		async: true,
		...
		success: function(data) {
			list = data.respBody;
			$("#realWhs").val(list.realWhs);
			$("#realWhs").attr("readonly", "readonly")
			$("#realNm").val(list.realNm);
			...
			$("input[name='realNm']").val(list.realNm);
			...
			$("select[name='province'] option:selected").text(list.prov);
			$("select[name='city'] option:selected").text(list.cty);
			$("select[name='town'] option:selected").text(list.cnty);
...
function IsCheckValue(realWhs, realNm, whsAddr, princ, deptName, tel, prov, cty, cnty) {
	if(realWhs == '') {
		alert("仓库编号不能为空")
		return false;
	} else if(realNm == '') {
		...
		alert("电话不能为空")
		return false;
	} else if(prov == '请选择' || prov == null) {
		alert("省或直辖市未选择")
		return false;
	} else if(cty == '请选择' || cty == null) {
		alert("市未选择")
		return false;
	}
	else if(cnty == '请选择' || cnty == null) {
		alert("县未选择")
		return false;
	}
	return true;
...
function editData() {
	...
	if(IsCheckValue(realWhs, realNm, whsAddr, princ, deptName, tel, prov, cty, cnty) == true) {
		...
		$.ajax({
			type: "post",
			url: url + "/mis/whs/whs_doc/updateRealWhs",
			async: true,
			data: saveJson,
			dataType: 'json',
...
function addData() {
	...
	if(IsCheckValue(realWhs, realNm, whsAddr, princ, deptName, tel, prov, cty, cnty) == true) {
		...
		var postData = JSON.stringify(data);
		$.ajax({
			url: url + '/mis/whs/whs_doc/insertRealWhs',
			type: 'post',
			data: postData,
			dataType: 'json',
			async: true,
...
$(function() {
	$(".delOrder").click(function() {
		...
		for(var i = 0; i < ids.length; i++) {
			...
			//选中行的id
			var jstime = $("#jqgrids").getCell(ids[i], "realWhs");
		...
		if(ids.length == 0) {
			...
			$.ajax({
				type: "post",
				url: url + "/mis/whs/whs_doc/deleteRealWhsList",
				async: true,
				data: deleteData,
				dataType: 'json',
				contentType: 'application/json',
