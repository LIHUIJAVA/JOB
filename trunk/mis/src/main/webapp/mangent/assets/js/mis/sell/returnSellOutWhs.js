...
$(function() {
	$('.print1').click(function() {
		...
		if(outWhsId == '') {
			...
			window.open("../../Components/sell/print_returnSellOut.html?1");
		}
	})
})
$(function() {
...
function searchsellSnglId(sellSnglId) {
	...
	$.ajax({
		...
		success: function(data) {
			...
			$("input[name='department']").val(data.respBody.deptName);
...
$(function() {
	...
	$("#jqgrids").jqGrid({
		...
		datatype: "json",
		colNames: [ '仓库编码','序号','仓库名称', '存货编码', '存货名称', '规格型号', '主计量单位', '主计量单位编码', '数量', '含税单价', '价税合计',
			'无税单价', '无税金额', '税率', '税额', '箱规', '箱数', '对应条形码', '批次',
			'生产日期', '失效日期', '保质期', '国际批次', '表体备注', '是否退货', '项目编码', '项目名称',
		], //jqGrid的列显示名字
		colModel: [{
				name: 'whsEncd',
				editable: true,
