...
//							var rowid = $("#zi_jqgrids").jqGrid('getGridParam', 'selrow');
						$("#zi_jqgrids").setRowData(j+1, {
							...
							srefCost: struSubList[j].srefCost
						})
					}
				}
...
//	   无税金额/含税单价/含税金额
	...
	mUprc = toDecimal(munTaxUprc * momQty);  // 无税金额
	if(isNaN(mUprc)) {
		mUprc = ""
	}
	
	taxAmtNum = toDecimal(mUprc * taxRate * 0.01); //税额
	if(isNaN(taxAmtNum)) {
		taxAmtNum = ""
	}
	
	mAmt = toDecimal(mUprc * 1 + taxAmtNum * 1);  // 含税金额
	if(isNaN(mAmt)) {
		mAmt = ""
	}
...
$(function() {
	...
	$("#zi_jqgrids").jqGrid({
		...
		colNames: ['仓库编码', '仓库名称', '存货编码*', '存货名称', '规格型号',
			...
			'税率', '含税单价', '含税金额', '无税单价', '无税金额', '税额', '箱规','参考成本'
		], //jqGrid的列显示名字
		colModel: [{
				name: 'whsEncd', //仓库编码
				editable: false,
...
function getJQAllData4() {
	...
	if(rowIds.length > 0) {
		for(var i = 0; i < rowIds.length; i++) {
			if(obj.getRowData(rowIds[i]).subQty == "") {
				continue;
			}else
			if(obj.getRowData(rowIds[i]).subQty != "") {
				arrayData.push(obj.getRowData(rowIds[i]).subQty * 1);
			}
		}
	}
	return arrayData;
}
function getJQAllData5() {
	//拿到grid对象
	var obj = $("#zi_jqgrids");
	//获取grid表中所有的rowid值
	var rowIds = obj.getDataIDs();
	...
	if(rowIds.length > 0) {
		for(var i = 0; i < rowIds.length; i++) {
			if(obj.getRowData(rowIds[i]).srefCost == "") {
				continue;
			}else
			if(obj.getRowData(rowIds[i]).srefCost != "") {
				arrayData.push(obj.getRowData(rowIds[i]).srefCost * 1);
			}
		}
	}
	return arrayData;
...
function SetNums2(rowid, cellname, val) {
	...
	var munTaxAmt = parseFloat(rowData.munTaxAmt);//母件无税金额
	var momQty = parseFloat(rowData.momQty);//母件数量
	var momSubRatio = parseFloat(rowDatas * momSubRatio); //母子比例
	
	var srefCost = parseFloat(rowDatas.srefCost); //子件参考成本
	var taxRate = parseFloat(rowDatas.taxRate); //税率
	
	var sunTaxAmt = parseFloat(rowDatas.sunTaxAmt); //子件无税金额
	var sunTaxUprc = parseFloat(rowDatas.sunTaxUprc); //子件无税单价
	var scntnTaxUprc = parseFloat(rowDatas.scntnTaxUprc); //子件含税单价
	var scntnTaxAmt = parseFloat(rowDatas.scntnTaxAmt); //子件含税金额
	...
	var list1 = getJQAllData5()
	var listNum1 = 0;//参考成本之和
	...
	for(var i = 0;i<listZ.length;i++) {
		var bxRule = listZ[i].bxRule; //箱规
		//无税金额  无税单价 含税单价 含税金额 箱数  子件数量
		var sAmt, sUprc, scUprc, scAmt, sbxQty, sbQty;
		...
		var num_z = listNum;
		var num_m = munTaxAmt / num_z * srefCost
		sAmt = toDecimal(num_m * sbQty * srefCost);//无税金额
		if(isNaN(sAmt)) {
			sAmt = ""
		}
		...
		scAmt = toDecimal(sAmt * 1 + staxAmtNum * 1);//含税金额
		if(isNaN(scAmt)) {
			scAmt = ""
		}
		
		scUprc = toDecimal(scAmt / sbQty);  // 含税单价
		if(isNaN(scUprc)) {
			scUprc = ""
		}
		sUprc = toDecimal(sAmt / sbQty);//无税单价
		if(isNaN(sUprc)) {
			sUprc = ""
		}
		$("#zi_jqgrids").setRowData(i + 1, {
...
function IsCheckValue(momKitEncd,momQty, mbatNum, listData) {
...
function SaveModifyData() {
	...
	//判断页面是否有值为空
	if(IsCheckValue(momKitEncd,momQty, mbatNum, listData) == true) {
