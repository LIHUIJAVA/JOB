...
function pageInit() {
	jQuery("#cust_jqgrids").jqGrid({
		...
		colNames: ['客户编码', '客户名称', '客户简称', '客户总公司', '联系人', '联系方式', '地址', '备注'], //jqGrid的列显示名字
		colModel: [{
				name: 'custId',
				align: "center",
				editable: true,
...
$(document).on('click', '.true', function() {
	...
	var rowDatas = $("#cust_jqgrids").jqGrid('getRowData', ids);
	if(ids.length == 0) {
		alert("请选择单据")
	} else {
		var custTotlCorpId = rowDatas.custId;
		var custTotlCorp = rowDatas.custNm;
		if(localStorage.cust=="1"){
			window.parent.opener.document.getElementById("custTotlCorpId").value = custTotlCorpId+"/"+custTotlCorp;
		}else if(localStorage.cust=="2"){
			window.parent.opener.document.getElementById("bllgCorp").value =custTotlCorp;
		}
...
$(document).on('click', '.false', function() {
	if(localStorage.cust=="1"){
			window.parent.opener.document.getElementById("custTotlCorpId").value = "";
		}else if(localStorage.cust=="2"){
			window.parent.opener.document.getElementById("bllgCorp").value ="";
		}
	window.close()
})